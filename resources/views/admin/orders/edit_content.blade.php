...
<div class="mb-3">
    ...
    @foreach(orderStatus() as $status => $label)
        <option value="{{ $status }}" {{ $model->order_status==$status?'selected':'' }}>{{ ucfirst($status) }}</option>
    @endforeach
    </select>
</div>
...
<div class="mb-3">
    ...
    <input type="text" name="tracking_number" id="tracking-id-input" value="{{ $model->tracking_number }}" placeholder="Enter order tracking ID" class="form-control" />
</div>
