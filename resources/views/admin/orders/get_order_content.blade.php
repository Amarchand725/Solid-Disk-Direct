...
<p>Customer: {{ $customerName ?? 'N/A' }} | Total: <strong>{{ currency() }}{{ $order->total }}</strong></p>
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>SubTotal</td>
            <td>
                <strong>{{ currency() }} <span id="sub-total">{{ number_format($order->subtotal, 2) }}</span></strong>
                <input type="hidden" name="sub_total_value" value="{{ number_format($order->subtotal, 2) }}">
            </td>
        </tr>
        <tr>
            <td colspan="5"></td>
        ...
        <tr>
            ...
            <td><input type="number" name="shipping_charges" class="form-control" value="{{ $order->shipping_cost }}"></td>
        </tr>
        <tr>
            <td colspan="5"></td>
            <td>Grand Total</td>
            <td>
                <strong>{{ currency() }} <span id="grand-total">{{ number_format($order->total, 2) }}</span></strong>
                <input type="hidden" name="grand_total_value" value="{{ number_format($order->total, 2) }}">
            </td>
        </tr>
    </tbody>
</table>
